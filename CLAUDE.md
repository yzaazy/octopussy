...
- `src/components/` — `SiteHeader.astro` (masthead, sync indicator, stud bar, filter controls, camera-scan button + scan results panel), `PartSection.astro` (collapsible `<details>` section), `PartCard.astro` (one part; carries the `data-key/needed/part/color` attributes the scripts rely on), `SiteFooter.astro` (download/backup buttons, build stamp).
- `src/scripts/` — client modules, `main.ts` is the only entry (wires the per-card steppers; the ✓ check-all button needs a second tap within 3s to confirm — it arms to an orange "?"): `state.ts` (progress state, server GET/PUT + localStorage mirror, sync indicator), `totals.ts` (card/counter/bar rendering incl. the qty badge — red ×N, orange "nog X" when partially owned, green when done — and section-complete detection), `filters.ts` (search/kleur/nog-nodig + sorting), `scan.ts` (camera scan: photo → Brickognize API → match candidates against the list; matches by part id only, not color), `collapse.ts` (remembered section collapse), `lights.ts` (kermis light program switcher + inloop sequencer), `confetti.ts`, `backup.ts` (missing-parts BrickLink XML, JSON backup import/export).
